...
class Model(object):
    ...
    def _train(self, epochs, batch_size):
        ...
        for _ in range(epochs):
            for batch_features, batch_labels in helper.batch_features_labels(self.data, self.labels, batch_size):
                samples_count += len(batch_features)
                self.session.run(self.optimizer, feed_dict={self.x: batch_features, self.y: batch_labels})
                loss += self.session.run(self.cost, feed_dict={self.x: batch_features, self.y: batch_labels})
    ...
    def _train_scaled(self, epochs, batch_size):
        ...
        samples_count = 0
        for _ in range(epochs):
            for features, labels, scale in helper.batch_features_labels_scale(self.data, self.labels, self.scale, batch_size):
                samples_count += len(features)
                self.session.run(self.optimizer, feed_dict={self.x: features, self.y: labels, self.z: scale})
                loss += self.session.run(self.cost, feed_dict={self.x: features, self.y: labels, self.z: scale})
        return loss/samples_count
